...
messagesRouter.use(requireHackathonAccess);

messagesRouter.get('/', requirePermission('message:view'), async (req: AuthRequest, res) => {
  try {
...
messagesRouter.post('/broadcast', requirePermission('message:broadcast'), async (req: AuthRequest, res) => {
  ...
  try {
    const teams = await prisma.team.findMany({
      where: { hackathonId, deletedAt: null, ...(teamIds === 'all' ? {} : { id: { in: teamIds } }) },
      select: { id: true, name: true, leaderPhone: true },
    });
    const message = await prisma.message.create({
      data: {
        content, channel, status: 'QUEUED', recipientType: teamIds === 'all' ? 'ALL' : 'SELECTED',
        hackathonId, sentById: req.user!.id,
        recipients: { create: teams.map((t) => ({ teamId: t.id, status: 'QUEUED' })) },
      },
      include: {
        sentBy: { select: { id: true, name: true } },
        recipients: { include: { team: { select: { id: true, name: true } } } },
...
messagesRouter.post('/:id/retry', requirePermission('message:broadcast'), async (req: AuthRequest, res) => {
  try {
    const message = await prisma.message.findUnique({
      where: { id: req.params.id },
      include: {
        recipients: { where: { status: 'FAILED' }, include: { team: true } },
      },
    });
